...
const whyChooseUs = [
	...
	},
	{
		...
		description:
			'Classes available 7 days a week with morning, afternoon, and evening options.',
		icon: Clock,
		color: 'text-[#F1894F]',
		bgColor: 'bg-[#F1894F]/10',
	},
	{
		...
		description:
			'Competitive rates with flexible payment plans and sibling discounts available.',
		icon: DollarSign,
		color: 'text-[#1F396D]',
		bgColor: 'bg-[#1F396D]/10',
	},
	{
		...
		description:
			'Small class sizes and one-on-one support to ensure every student thrives.',
		icon: HeartHandshake,
		color: 'text-[#F16112]',
		bgColor: 'bg-[#F16112]/10',
	},
	...
	useEffect(() => {
		...
		const animate = () => {
			...
			const currentCount = Math.floor(
				startCount + (endValue - startCount) * progress
			);
	...
	return (
		<Card className="text-center p-6 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 rounded-2xl">
			<CardContent className="p-0">
				...
				<div className={`text-3xl font-bold ${stat.color} mb-2`}>0</div>
				<p className="text-gray-600 font-medium">{stat.label}</p>
			</CardContent>
		</Card>
	);
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
				...
				<div className="text-center mb-16">
					...
					<p className="text-lg text-gray-600 max-w-2xl mx-auto">
						We&apos;re committed to providing exceptional education that empowers
				...
				<div id="stats-section" className="mb-20">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-8">
						<ClientOnly
							fallback={statisticsData.map((stat, index) => {
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
					{whyChooseUs.map((item) => {
						...
						return (
							<Card
								key={item.id}
								className="p-6 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 rounded-2xl"
							>
								<CardContent className="p-0">
									<div
