...
export default function HomeClient({ initialData }: HomeClientProps) {
  ...
  const { data: storeData, loading, error } = useSelector((s: RootState) => s.home);
  const data = storeData ?? initialData ?? null;
  ...
  const popularCourses = useMemo(
    () => (data?.popularCourses || []).map((c) => ({ ...c, IconComponent: getIconComponent(c.icon) })),
    [data]
  );
  ...
  const k12Programs = useMemo(
    () => (data?.k12Programs || []).map((p) => ({ ...p, IconComponent: getIconComponent(p.icon) })),
    [data]
  );

  const steamPrograms = useMemo(
    () => (data?.steamPrograms || []).map((p) => ({ ...p, IconComponent: getIconComponent(p.icon) })),
    [data]
  );
  ...
  const whyChooseUs = useMemo(
    () => (data?.whyChooseUs || []).map((w) => ({ ...w, IconComponent: getIconComponent(w.icon) })),
    [data]
  );
  ...
  const showSkeleton =
    !initialData && (loading || !data || (data && (!data.heroSlides || data.heroSlides.length === 0)));
