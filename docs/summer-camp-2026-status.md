...
1. **`src/app/[locale]/camps/summer/page.tsx`** — Meta Pixel: dynamic `import('@/lib/meta-pixel')` for guide submit; mobile (`max-width: 768px`) longer idle timeouts for FAQ fetch and SlotsPanel (`SummerCampUI`) preload; hero `Image` `quality={70}`, mobile sticky logo `fetchPriority="low"` + `decoding="async"`; hero/subhead/trust text contrast (`zinc-100`, brochure `text-white`); primary CTAs / sticky bar use **`#047857` / `#065f46`**; active filter chips **`#146c43`**; **`SummerCampGuideLeadDialog`** loaded only after first open or `#lead-capture` / `#summercamp` (smaller initial JS).
